...
//***************************************************************************************************************
      bool trackIDused[100]; for (int x = 0; x < 100; x++){trackIDused[x] = false;}
//***************************************************************************************************************
...
//***************************************************************************************************************
	trackIDused[jt->second->id] = true;
//***************************************************************************************************************
      }

//***************************************************************************************************************
CvID minTrackIDUnused = 0;

for (int x = 0; x < 100; x++){ if (trackIDused[x] == false) {minTrackIDUnused = x; break;}}
...
//***************************************************************************************************************
	  if (nTracks < 100) track->id = minTrackIDUnused;
	  else{maxTrackID++;track->id = maxTrackID;}

	  //maxTrackID++;track->id = maxTrackID;
//***************************************************************************************************************
	  ...
	  track->angle          = blob->angle;
	  track->angleDegrees   = blob->angleDegrees;

	  track->lifetime = 0;
	  track->active = 0;
	  track->inactive = 0;
//***************************************************************************************************************
	  if (nTracks < 100) tracks.insert(CvIDTrack(minTrackIDUnused, track));
	  else tracks.insert(CvIDTrack(maxTrackID, track));
...
//***************************************************************************************************************
	...
	if (c)
	{
	  std::list<CvTrack*> tt; tt.push_back(T(j));
	  std::list<CvBlob*> bb;
	  ...
	  CvTrack *track;
	  unsigned int area = 0;
	  for (std::list<CvTrack*>::const_iterator it=tt.begin(); it!=tt.end(); ++it)
	  {
	    CvTrack *t = *it;
	  ...
	  area = 0;
	  //cout << "Matching blobs: ";
	  for (std::list<CvBlob*>::const_iterator it=bb.begin(); it!=bb.end(); ++it)
	  {
	    CvBlob *b = *it;
	  ...
	  track->label = blob->label;
	  track->lastCentroid = track->centroid;
	  track->centroid = blob->centroid;
