...
#define imageOut(X, Y) imgDataOut[(X) + (Y)*stepOut]
	...
	for (unsigned int x=0; x<imgIn_width; x++)
	{
      ...
      for (CvBlobs::iterator it=blobs.begin(); it!=blobs.end(); ++it){
		  ...
		  cvCentralMoments((*it).second, imgOut);
        	  double angle = cvAngle((*it).second);
		  double angleDegrees = angle * (double)180 /(double)3.1415926;
		  (*it).second->angle = angle;
		  (*it).second->angleDegrees = angleDegrees;
      }
