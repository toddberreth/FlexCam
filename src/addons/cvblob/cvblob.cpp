...
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See thefile:///home/calypso/Desktop/flexcam/libs/cvblob/cvtrack.cpp
...
namespace cvb
{
  ...
  void cvFilterByRegion(CvBlobs &blobs, unsigned int x, unsigned int y, unsigned int width, unsigned height)
  {
    ...
    while(it!=blobs.end())
    {
      ...
      if (
	(blob->centroid.x < x) || (blob->centroid.x > x+width) ||
	(blob->centroid.y < y) || (blob->centroid.y > y+height)
	)
      {
	cvReleaseBlob(blob);
  ...
  void cvConstrainBlobMinMax(CvBlobs &blobs, unsigned int x, unsigned int y, unsigned int width, unsigned height)
  {
    ...
    {
      	CvBlob *blob=(*it).second;
      	if (blob->minx < x) {blob->minx = x;}
      	if (blob->maxx > x + width) {blob->maxx = x + width;}
	if (blob->miny < y) {blob->miny = y;}
      	if (blob->maxy > y + height) {blob->maxy = y + height;}
	++it;
    }
  }
  ...
  void cvRenderBlob(const IplImage *imgLabel, CvBlob *blob, IplImage *imgSource, IplImage *imgDest, unsigned short mode, CvScalar const &color, double alpha)
  {
	...
	std::cout << std::endl;
      ...
      if (mode&CV_BLOB_RENDER_BOUNDING_BOX) {
           ...
           // cout << blob->minx << " " << blob->miny << " " << blob->maxx << " " << blob->maxy << endl;
      }
...
#define _HSV2RGB_(H, S, V, R, G, B) \
  ...
  void cvRenderBlobs(const IplImage *imgLabel, CvBlobs &blobs, IplImage *imgSource, IplImage *imgDest, unsigned short mode, double alpha)
  {
	...
	for (CvBlobs::const_iterator it=blobs.begin(); it!=blobs.end(); ++it)
	{
	  //CvScalar test = cvBlobMeanColor((*it).second, imgLabel, imgSource);
	 ...
	 pal[label] = CV_RGB(255, 255, 255);       // edited this temporarily set all blobs to render white
	}
      }
